 ...
 * @returns {string}
 */
function toWords(inputNumber, asOrdinal) {
    var words;
    var num = parseInt(inputNumber, 10);
    if (!isFinite(num)) {
        throw new TypeError('Not a finite number: ' + inputNumber + ' (' + typeof inputNumber + ')');
    }
    if (!isSafeNumber(num)) {
...
function generateWords(number, wordsArray) {
